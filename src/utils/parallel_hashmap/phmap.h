...
//
// The table interface is similar to that of phmap::flat_hash_set. Notable
...
public:
    ...
    // argument like phmap::flat_hash_set<value_type> does, we have two overloads
    ...
    // For example:
    //
    //   phmap::flat_hash_set<ArenaString> s;
    ...
    // Erases the element pointed to by `it`.  Unlike `phmap::flat_hash_set::erase`,
...
// Its interface is similar to that of `phmap::flat_hash_set<T>` with the
...
// has been optimized for both speed and memory footprint in most common use
// cases. Its interface is similar to that of `phmap::flat_hash_set<T>` with the
// following notable differences:
//
// * Supports heterogeneous lookup, through `find()`, `operator[]()` and
//   `insert()`, provided that the map is provided a compatible heterogeneous
